...
def draw_bbox(img, result,label_list, threshold=0.45): #threshold =0.5 original scores
...
#cv2.rectangle(img, (int(xmin-30),int(ymin-50)), (int(xmax+50), int(ymax+50)), (255,0,0), 5)
        ...
        try:
            ...
            cv2.rectangle(img, (int(xmin-30),int(ymin-30)), (int(xmax+30), int(ymax+30)), (255,0,0), 5)
            cv2.putText(img, text, (int(xmin-50), int(ymin-50)), cv2.FONT_HERSHEY_SIMPLEX, 4, (0,0,255), 5)
            #cv2.putText(img, str(round(score,2)), (int(xmin-35), int(ymin-2)), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,255,0), 2)
        except KeyError:
            pass
